...
adsorption_data = pd.read_csv(dir_name/'core_n2_adsorption.csv', index_col=0)

aim_y = adsorption_data.columns 
dataset_x = pd.concat([struct_data, discriptor_data], axis=1)
same_idx = set(list(dataset_x.index)) & set(list(adsorption_data.index))
dataset_x = dataset_x.loc[same_idx, :]

model_names = ['PLS', 'Ridge', 'LASSO', 'EN', 'SVR(Linear)', 'SVR(rbf)', 'RandomForest', 'GP', 'LightGBM']

fold_number = 10
max_number_of_components = 80
do_autoscaling = True
random_state=3
...
def evaluation(raw_y, estimated_y, data_type, savefig=False):
    ...
    plt.savefig(
                save_dir+'{}_{}_{}.png'.format(col, data_type, model_names[i]), 
                bbox_inches='tight'
                )
    plt.show()
...
for col in aim_y:
    ...
    score_metrics_train = score_metrics_test = score_metrics.copy()

    y_data = adsorption_data.loc[same_idx, col]
    ...
    score_metrics_train.to_csv(save_dir+'predict_scores_train_{}'.format(col))
    score_metrics_test.to_csv(save_dir+'predict_scores_test_{}'.format(col))
...
with open(save_dir/'estimated_y_train.json', 'w') as f:
    json.dump(estimated_train_alls, f)
with open(save_dir/'estimated_y_test.json', 'w') as f:
    json.dump(estimated_test_alls, f)
